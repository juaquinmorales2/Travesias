    ...
    const handleMigration = async () => {
        ...
        try {
            ...
            for (const event of eventsData) {
                logMsg(`Procesando Evento: ${event.title}...`);
                ...
                // Image Upload
                if (event.file_source && (!existingEvent || !existingEvent.image_url)) {
                    try {
                        ...
                        if (!uploadError) {
                            ...
                            logMsg(`- Imagen subida: ${event.file_name}`);
                        }
                    } catch (e: any) { logMsg(`- Error imagen: ${e.message}`); }
                } else if (existingEvent?.image_url) {
                    event.image_url = existingEvent.image_url;
                }

                // Manual PDF Upload
                const manualSource = (event as any).manual_source;
                if (manualSource && (!existingEvent || !existingEvent.manual_url)) {
                    try {
                        const response = await fetch(manualSource);
                        const blob = await response.blob();
                        const manualName = (event as any).manual_name;
                        const file = new File([blob], manualName, { type: blob.type });
                        const fileName = `manuals/${manualName}`;
                        ...
                        if (!uploadError) {
                            ...
                            logMsg(`- Manual subido: ${manualName}`);
                        }
                    } catch (e: any) { logMsg(`- Error PDF: ${e.message}`); }
                } else if (existingEvent?.manual_url) {
                    event.manual_url = existingEvent.manual_url;
                }
                ...
                if (existingEvent) {
                    ...
                    if (updateError) logMsg(`- Error actualizando: ${updateError.message}`);
                    else logMsg(`- Evento actualizado.`);
                } else {
                    const { error: insertError } = await supabase
                        .from('events')
                        .insert([dbEvent]);

                    if (insertError) logMsg(`- Error creando: ${insertError.message}`);
                    else logMsg(`- Evento creado.`);
                }
            }

            // 4. Migrate Modal
            logMsg('Procesando Modal...');
            let modalImageUrl = '';
            let modalManualUrl = '';
            ...
            try {
                ...
                if (!error) modalImageUrl = supabase.storage.from('images').getPublicUrl(fileName).data.publicUrl;
            } catch (e) { }
            ...
            try {
                ...
                const { error } = await supabase.storage.from('images').upload(fileName, file, { upsert: true });
                if (!error) modalManualUrl = supabase.storage.from('images').getPublicUrl(fileName).data.publicUrl;
            } catch (e) { }
            ...
            const { data: existingModal } = await supabase.from('modals').select('id').eq('title', 'Promo Inicio').maybeSingle();

            if (existingModal) {
                const { error } = await supabase.from('modals').update(modalData).eq('id', existingModal.id);
                if (error) logMsg(`Error actualizando Modal: ${error.message}`);
                else logMsg('Modal actualizado.');
            } else {
                const { error } = await supabase.from('modals').insert([modalData]);
                if (error) logMsg(`Error creando Modal: ${error.message}`);
                else logMsg('Modal creado.');
            }
                ...
                <div className="bg-gray-800 p-6 rounded-xl border border-gray-700 mb-6">
                    ...
                    <div className="bg-yellow-500/10 border border-yellow-500/20 rounded p-4 mb-4">
                        <p className="text-yellow-400 text-sm">
                            <strong>Nota:</strong> Si ya existen datos, esta herramienta intentará actualizar los archivos faltantes (PDFs, imágenes) y restaurar el Modal original.
                        </p>
                    </div>
                    <button
                    ...
                    >
                        {migrating ? 'Migrando...' : 'Forzar Migración / Restauración'}
                    </button>
                </div>
