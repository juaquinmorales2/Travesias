...
// --- Datos de eventos ---
const flyers = [
  { id: 1, title: "8ª Edición Travesía Internacional Laguna del Sauce", date: "20 de diciembre 2025", location: "Punta del Este", img: ejemplo1, inscripcion: "https://forms.gle/ih7nsjkL13HoXAZm8", manual: LagunaPDF },
  ...
  { id: 8, title: "Gala de Premiación 2026", date: "03 de abril 2026", location: "", img: ejemplo8 },
];
...
export default function CalendarioEventos() {
  ...
  return (
    <section className="py-12 bg-gradient-to-b from-black to-gray-900 text-white">
      <div className="max-w-3xl md:max-w-md mx-auto px-4">
        ...
        <div className="flex flex-col gap-12">
          {months.map(({ date, events }) => (
            <article key={date.toISOString()} className="bg-gradient-to-b from-gray-800/40 to-black/40 rounded-2xl p-6 shadow-2xl">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-xl md:text-2xl font-semibold">{formatMonthYear(date)}</h3>
              </div>
                ...
                {events.map((ev, i) => (
                  <div key={ev.id} onClick={() => setSelectedEvent(ev)} className="relative rounded-2xl overflow-hidden border-4 border-teal-400 cursor-pointer hover:scale-[1.02] transition-transform opacity-0 translate-y-6 animate-[fadeInUp_0.7s_ease-out_forwards]" style={{ animationDelay: `${i * 0.2}s` }}>
                    ...
                    <div className="absolute inset-0 bg-black/40 flex flex-col justify-between p-4">
                      ...
                      <div>
                        <h4 className="font-bold text-3xl" style={{ color: "white", textShadow: `-1px -1px 0 #000, 1px -1px 0 #000, -1px 1px 0 #000, 1px 1px 0 #000` }}>{ev.title}</h4>
                        <p className="text-sm opacity-80" style={{ color: "white", textShadow: `-1px -1px 0 #000, 1px -1px 0 #000, -1px 1px 0 #000, 1px 1px 0 #000` }}>{ev.location}</p>
                      </div>
                    </div>
                  </div>
                ))}
      ...
      {selectedEvent && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-[fadeIn_0.3s_ease-out_forwards]">
          <div className="relative bg-gray-900 rounded-2xl overflow-hidden max-w-lg w-full max-h-[120vh] shadow-2xl animate-[scaleUp_0.4s_ease-out_forwards] flex flex-col">
            <button onClick={() => setSelectedEvent(null)} className="absolute top-3 right-3 bg-black/50 hover:bg-black/70 text-white p-2 rounded-full z-10">
              <X size={20} />
            </button>
            ...
            <div className="overflow-y-auto p-5 flex-1 flex flex-col gap-4">
              ...
              {selectedEvent.location && <p className="text-sm text-gray-400">{selectedEvent.location}</p>}
              ...
              <div className="flex flex-col items-center justify-center gap-4 mt-4">
                ...
                <a href={`https://wa.me/59899930821?text=${encodeURIComponent(`Hola, quiero más información sobre el evento "${selectedEvent.title}" del ${selectedEvent.date}.`)}`} target="_blank" rel="noopener noreferrer" className="w-full bg-green-600 hover:bg-green-700 transition rounded-xl px-4 py-3 text-white font-semibold text-lg text-center shadow-lg flex items-center justify-center gap-3">
